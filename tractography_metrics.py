...
def main():
    tractogram, txt_filepath, fa_filepath, bzero_filepath, md_filepath, header, to_csv, to_xlsx = setup()
...
def setup():
    ...
    parser.add_argument('-md', '--Mean_Diffusivity', help='Name of the input mean diffusivity image file',
                        type=check_nii)
    parser.add_argument('-h', '--header', help='Add header information tp the text file.', action=check_str)
    ...
    return (args.Input_Tractogram, args.Output_Stats, args.Fractional_Anisotropy, args.b_zero, args.Mean_Diffusivity,
            args.header, args.save_csv, args.save_xlsx)
...
def check_txt(value):
    if value.endswith('.txt') and os.path.isfile(os.path.abspath(value)):
        return value
    else:
